...
let _confirmDeleteJobId: string | null = null;

export function renderCronSimple(props: CronProps) {
  const confirmJob = _confirmDeleteJobId
    ? props.jobs.find((j) => j.id === _confirmDeleteJobId)
    : null;

  const setConfirm = (id: string | null) => {
    _confirmDeleteJobId = id;
    props.onRefresh();
  };
  ...
  return html`
    ${confirmJob
      ? html`
          <div class="cqc-overlay" @click=${(e: Event) => { if (e.target === e.currentTarget) setConfirm(null); }}>
            <div class="cqc-modal cron-delete-modal">
              <div class="cron-delete-modal__icon">
              ...
              <div class="cron-delete-modal__title">Remove reminder?</div>
              <div class="cron-delete-modal__body">"${confirmJob.name}" will be permanently deleted.</div>
              <div class="cron-delete-modal__actions">
                <button class="btn" @click=${() => setConfirm(null)}>Cancel</button>
                <button
                  class="btn btn--danger"
                  ?disabled=${props.busy}
                  @click=${() => { setConfirm(null); props.onRemove(confirmJob); }}
                >Remove</button>
              </div>
            </div>
          </div>
    ...
    ${props.jobs.length === 0
      ? html`<div class="muted">No reminders yet. Hit New to add one.</div>`
      : html`<div class="simple-job-list">${props.jobs.map((job) => renderSimpleJob(job, props, setConfirm))}</div>`}
    ${props.jobsHasMore
      ? html`
          <div class="row" style="margin-top: 12px">
            <button
...
function humanizeCronExpr(expr: string): string {
  ...
  const allDom = dom === "*", allMonth = month === "*", allDow = dow === "*";
  ...
  const simpleInt = (s: string) =>
    !s.includes("*") && !s.includes(",") && !s.includes("-") && !s.includes("/")
      ? parseInt(s)
      : NaN;

  const h = simpleInt(hour), m = simpleInt(min);
  const hasSimpleTime = !isNaN(h) && !isNaN(m);

  // Every minute
  if (min === "*" && hour === "*" && allDom && allMonth && allDow) return "Every minute";

  // Every N minutes: */N * * * *
  if (min.startsWith("*/") && hour === "*" && allDom && allMonth && allDow)
    return `Every ${min.slice(2)} minutes`;

  // Every hour (on the hour or at :MM): * or 0 * * * *
  if (hour === "*" && allDom && allMonth && allDow)
    return !isNaN(m) ? `Every hour at :${String(m).padStart(2, "0")}` : "Every hour";

  // Every N hours: 0 */N * * *
  if (hour.startsWith("*/") && allDom && allMonth && allDow) {
    const n = hour.slice(2);
    return !isNaN(m) && m !== 0
      ? `Every ${n} hours at :${String(m).padStart(2, "0")}`
      : `Every ${n} hours`;
  }

  if (hasSimpleTime && allMonth) {
    const time = fmtTime(h, m);

    // Daily
    if (allDom && allDow) return `Daily at ${time}`;
    // Weekdays
    if (allDom && dow === "1-5") return `Weekdays at ${time}`;
    // Weekends
    if (allDom && (dow === "0,6" || dow === "6,0" || dow === "6,7" || dow === "0,7"))
      return `Weekends at ${time}`;
    // Single named day
    if (allDom && DAY[dow]) return `${DAY[dow]}s at ${time}`;
    // Multiple named days e.g. 1,3,5
    if (allDom && /^[\d,]+$/.test(dow)) {
      const names = dow.split(",").map((d) => DAY[d]).filter(Boolean);
      if (names.length === dow.split(",").length)
        return `${names.join(", ")} at ${time}`;
    }
    // Monthly on Nth
    if (allDow && !dom.includes("*") && !dom.includes(",") && !dom.includes("/")) {
  ...
  // Fallback: show the raw expression — at least it's honest
  return expr;
}

function friendlySchedule(job: CronJob): string {
  const s = job.schedule;
  if (s.kind === "at") {
    const atMs = Date.parse(s.at);
    return Number.isFinite(atMs) ? `Once at ${formatMs(atMs)}` : `Once at ${s.at}`;
  }
  if (s.kind === "every") return formatCronSchedule(job);
  return humanizeCronExpr(s.expr);
}
...
function renderSimpleJob(job: CronJob, props: CronProps, onRequestDelete: (id: string) => void) {
  const schedule = friendlySchedule(job);
  const prompt = jobPrompt(job);

  return html`
    <div class="simple-job">
      <div class="simple-job__top">
        <div class="simple-job__name">${job.name}</div>
        <div class="simple-job__top-actions">
          <button
            class="job-trash-btn"
            ?disabled=${props.busy}
          ...
          <label class="job-toggle">
            ...
            <span class="job-toggle__label">${job.enabled ? "On" : "Off"}</span>
          </label>
        </div>
      </div>
      ${prompt ? html`<div class="simple-job__prompt">${prompt}</div>` : nothing}
      <span class="simple-job__schedule">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor">
          <path fill-rule="evenodd" d="M1 8a7 7 0 1 1 14 0A7 7 0 0 1 1 8Zm7-3a.75.75 0 0 1 .75.75v2.69l1.03 1.03a.75.75 0 0 1-1.06 1.06l-1.25-1.25a.75.75 0 0 1-.22-.53V5.75A.75.75 0 0 1 8 5Z" clip-rule="evenodd"/>
        </svg>
        ${schedule}
      </span>
    </div>
